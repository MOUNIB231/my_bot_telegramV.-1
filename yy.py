...
def user_is_activated(user_id):
    ...
    if not user:
        return False
    if len(user) > 7 and user[7]:  # blocked
        return False
...
def process_referral(user_id, referral_code):
    ...
    if referrer and referrer[0] != user_id:
        ...
        # مكافأة الإحالة: إذا أحال 3 مستخدمين، يمدد يوم واحد
...
async def check_expiring_accounts(context: ContextTypes.DEFAULT_TYPE):
    """فحص الحسابات التي ستنتهي صلاحيتها خلال 24 ساعة وإرسال تنبيهات"""
    ...
    for u_id, level, expiry in users:
        try:
            ...
            await context.bot.send_message(u_id,
                f"⚠️ *تنبيه الصلاحية*\n\n"
                f"👋 مرحباً!\n"
                f"📅 صلاحيتك ستنتهي خلال {hours} ساعة\n"
                f"⭐ مستواك: {LEVEL_NAMES.get(level, level)}\n\n"
                f"🔄 للتجديد، اطلب كود تفعيل جديد.",
                parse_mode=ParseMode.MARKDOWN)
        except:
            pass

async def check_scheduled_broadcasts(context: ContextTypes.DEFAULT_TYPE):
    """فحص وإرسال الإعلانات المجدولة"""
    ...
    for b_id, msg in broadcasts:
        ...
        for u in users:
            try:
                await context.bot.send_message(u[0], f"📣 *إعلان مجدول:*\n\n{msg}", parse_mode=ParseMode.MARKDOWN)
                count += 1
            except:
                pass
        db_execute("UPDATE scheduled_broadcasts SET sent=1 WHERE id=?", (b_id,))
        db_execute("INSERT INTO broadcast_log (message, recipients_count, sent_time) VALUES (?,?,?)", (msg, count, now))

async def send_daily_usage_report(context: ContextTypes.DEFAULT_TYPE):
    """إرسال تقرير الاستخدام اليومي للأدمن"""
    today = datetime.now().strftime("%Y-%m-%d")
    stats = db_fetchone("SELECT total_ai_requests, new_users, new_activations FROM daily_stats WHERE date=?", (today,))
    if stats:
        ai, new_u, new_a = stats
        await context.bot.send_message(ADMIN_ID,
            f"📊 *التقرير اليومي*\n\n"
            f"📅 التاريخ: {today}\n"
            f"🧠 أسئلة AI: {ai or 0}\n"
            f"👥 مستخدمين جدد: {new_u or 0}\n"
            f"✅ تفعيلات جديدة: {new_a or 0}",
            parse_mode=ParseMode.MARKDOWN)

async def send_usage_warning(user_id, context):
    """إرسال تنبيه عند وصول المستخدم لحد الأسئلة اليومي"""
    level = get_user_level(user_id)
    limit = LEVEL_LIMITS.get(level, 10)
    try:
        await context.bot.send_message(user_id,
            f"⚠️ *تنبيه الاستخدام*\n\n"
            f"📊 وصلت لحد الأسئلة اليومي ({limit} سؤال)\n"
            f"⭐ مستواك: {LEVEL_NAMES.get(level, level)}\n\n"
            f"🔄 للترقية لمستوى أعلى، تواصل مع المشرف.",
            parse_mode=ParseMode.MARKDOWN)
    except:
        pass
...
def ask_ai(prompt: str, mode: str = 'general') -> str:
    if USE_MOCK_AI:
        return "🧠 الذكاء الاصطناعي في وضع التجربة. فعّل المفتاح الحقيقي."
    try:
        ...
        if "choices" in data and len(data["choices"]) > 0:
            ...
            return "❌ رد غير متوقع. تأكد من صحة مفتاح Groq."
...
def main_menu_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("🔐 تشفير AES", callback_data="menu_encrypt"),
         InlineKeyboardButton("🔓 فك AES", callback_data="menu_decrypt")],
...
def ai_modes_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("💻 مبرمج", callback_data="aimode_coder"),
         InlineKeyboardButton("📝 كاتب", callback_data="aimode_writer")],
        [InlineKeyboardButton("🎨 مبدع", callback_data="aimode_creative"),
         InlineKeyboardButton("🌍 مترجم", callback_data="aimode_translator")],
...
def advanced_tools_keyboard():
    keyboard = [
        [InlineKeyboardButton("📦 Base64 تشفير", callback_data="tool_base64enc"),
         InlineKeyboardButton("📦 Base64 فك", callback_data="tool_base64dec")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not existing:
        ...
        # معالجة الإحالة
        if args:
            process_referral(user_id, args[0])
    else:
    ...
    if user_is_activated(user_id):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🔹 لدي كود", callback_data="use_code")],
        ]
        await update.message.reply_text(
            "🔒 *حسابك غير مفعّل*\nاختر أحد الخيارين:",
...
async def request_activation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_is_activated(user_id):
        ...
        return
    db_execute("INSERT OR REPLACE INTO pending_activations VALUES (?,?,?,?)",
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        keyboard = [
            [InlineKeyboardButton("🥉 يوم - عادي", callback_data=f"approvewith_{target_id}_1_basic")],
        ...
        try:
            await context.bot.send_message(target_id, "❌ تم رفض طلبك")
        except:
            pass
...
async def admin_approve_with_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    days = int(days_str)
    code = generate_code()
    db_execute("INSERT INTO activation_codes (code, created_by, validity_days, created_at, level) VALUES (?,?,?,?,?)",
               (code, ADMIN_ID, days, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), level))
    try:
...
async def main_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_is_activated(user_id):
        await query.edit_message_text("🔒 غير مفعّل")
        return
    ...
    if data == "menu_ai":
        ...
        if not can_ask:
            await send_usage_warning(user_id, context)
            await query.edit_message_text(f"⚠️ وصلت حد الأسئلة اليومي. عد غداً.\n⭐ مستواك: {LEVEL_NAMES.get(get_user_level(user_id))}", reply_markup=main_menu_keyboard(), parse_mode=ParseMode.MARKDOWN)
            return
        ...
        tool = data.split("_")[1]
        tool_names = {'base64enc': 'Base64 تشفير', 'base64dec': 'Base64 فك', 'md5': 'MD5 Hash', 'sha256': 'SHA256 Hash'}
        state_map = {'base64enc': 'base64_enc', 'base64dec': 'base64_dec', 'md5': 'md5_hash', 'sha256': 'sha256_hash'}
        await query.edit_message_text(f"📦 *{tool_names.get(tool, tool)}*\nأرسل النص:", reply_markup=back_to_main_button(), parse_mode=ParseMode.MARKDOWN)
        USER_STATES[user_id] = state_map.get(tool)
        ...
        text = (f"👥 *نظام الإحالة*\n\n"
                f"🔗 رابطك:\n`{link}`\n\n"
                f"👤 المدعوين: {ref_count}\n"
                f"🎁 3 مدعوين = يوم مجاني!")
        ...
        if not history:
            await query.edit_message_text("📜 لا يوجد سجل", reply_markup=main_menu_keyboard())
        else:
            text = "📜 *آخر 5 محادثات:*\n\n"
            for q, a, m, t in history:
        ...
        text = (f"👤 *ملفك الشخصي*\n\n"
                f"🆔 ID: {user_id}\n"
                f"⭐ المستوى: {LEVEL_NAMES.get(level, level)}\n"
                f"🧠 أسئلة اليوم: {usage}/{limit}\n"
                f"📊 إجمالي الأسئلة: {total_ai}\n"
                f"📅 الصلاحية: {expiry}")
        await query.edit_message_text(text, reply_markup=main_menu_keyboard(), parse_mode=ParseMode.MARKDOWN)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif state == "base64_enc":
        result = base64_encode(update.message.text)
        await update.message.reply_text(f"📦 *Base64:*\n`{result}`", parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu_keyboard())
        USER_STATES.pop(user_id)
    
    elif state == "base64_dec":
        result = base64_decode(update.message.text)
        await update.message.reply_text(f"📦 *مفكوك:*\n`{result}`", parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu_keyboard())
        USER_STATES.pop(user_id)
    
    elif state == "md5_hash":
        result = md5_hash(update.message.text)
        await update.message.reply_text(f"🔑 *MD5:*\n`{result}`", parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu_keyboard())
        USER_STATES.pop(user_id)
    
    elif state == "sha256_hash":
        result = sha256_hash(update.message.text)
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_stats":
        ...
        if not reqs:
            await query.edit_message_text("📝 لا طلبات", reply_markup=admin_menu_keyboard())
        else:
        ...
        keyboard = [
            [InlineKeyboardButton("🥉 يوم - عادي", callback_data="gencode_1_basic")],
        ...
        code = generate_code()
        db_execute("INSERT INTO activation_codes VALUES (?,?,?,NULL,NULL,?,0,?)",
                   (code, ADMIN_ID, int(days), datetime.now().strftime("%Y-%m-%d %H:%M:%S"), level))
...
async def admin_change_level_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if USER_STATES.get(user_id) != "admin_change_level_id":
        return
    try:
        target = int(update.message.text)
    except:
        ...
        return
    keyboard = [
        [InlineKeyboardButton("🥉 عادي", callback_data=f"setlevel_{target}_basic")],
...
def main():
    ...
    job_queue.run_repeating(check_expiring_accounts, interval=3600, first=60)  # كل ساعة
    job_queue.run_repeating(check_scheduled_broadcasts, interval=300, first=30)  # كل 5 دقائق
    job_queue.run_daily(send_daily_usage_report, time=datetime.strptime("23:59", "%H:%M").time())  # يومياً 11:59 م
    ...
    print("✅ MonoAIHub bot is running with notification system...")
    app.run_polling()
