...
import httpx  # مثبتة مسبقاً مع python-telegram-bot

# ==================== الإعدادات العامة ====================
TOKEN = "8612117067:AAGGTV8QLmuEi7m3xGkNbuiZn5UJNtTwiwY"                     #كن البوت من BotFather
...
# ==================== إعدادات Groq (LLaMA) ====================
GROQ_API_KEY = "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"                        # ضع مفتاح Groq الخاص بك هنا
...
def aes_decrypt(encrypted_text: str) -> str:
    try:
        ...
        return f"❌ فشل فك التشفير"

# ==================== الذكاء الاصطناعي (Groq عبر httpx) ====================
def ask_ai(prompt: str) -> str:
    if USE_MOCK_AI:
        return "الذكاء الاصطناعي غير مفعّل حالياً."
    try:
        response = httpx.post(
...
def admin_menu_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("📝 طلبات التفعيل المعلقة", callback_data="admin_requests")],
        [InlineKeyboardButton("➕ إنشاء كود تفعيل", callback_data="admin_create_code")],
        [InlineKeyboardButton("🚫 حظر / فك حظر مستخدم", callback_data="admin_block")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_is_activated(user_id):
        ...
        await update.message.reply_text(
            "حسابك غير مفعّل. اختر أحد الخيارين:",
...
async def request_activation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"📩 طلب تفعيل جديد:\nالمستخدم: @{user.username} ({user.full_name})\nID: `{user_id}`"
    ...
    await query.edit_message_text("✅ تم إرسال طلبك إلى المشرف. يرجى الانتظار.")
    ...
    if action == "approve":
        ...
        try:
            await context.bot.send_message(target_id,
                f"✅ تمت الموافقة على تفعيلك!\nكود التفعيل: `{code}`\nصلاحيته: {validity} يوم.\nاستخدم زر 'لدي كود تفعيل' لإدخاله.",
                parse_mode=ParseMode.MARKDOWN)
        except:
            pass
        await query.edit_message_text(f"تمت الموافقة على المستخدم {target_id} وإرسال الكود له.")
        db_execute("DELETE FROM pending_activations WHERE user_id=?", (target_id,))
    elif action == "reject":
        try:
            await context.bot.send_message(target_id, "❌ تم رفض طلب التفعيل الخاص بك.")
        except:
            pass
        await query.edit_message_text(f"تم رفض طلب المستخدم {target_id}.")
        db_execute("DELETE FROM pending_activations WHERE user_id=?", (target_id,))

async def use_code_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("الرجاء إدخال كود التفعيل:")
...
async def handle_code_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ تم التفعيل بنجاح! الكود صالح {validity_days} يوم.",
        reply_markup=main_menu_keyboard()
    )
...
async def main_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_ai":
        await query.edit_message_text("أرسل سؤالك وسيتم الرد عليك:", reply_markup=back_to_main_button())
        USER_STATES[user_id] = "ai_question"
    elif data == "menu_encrypt":
        await query.edit_message_text("أرسل النص الذي تريد تشفيره:", reply_markup=back_to_main_button())
        USER_STATES[user_id] = "encrypt_text"
    elif data == "menu_decrypt":
        await query.edit_message_text("أرسل النص المشفر بصيغة JSON:", reply_markup=back_to_main_button())
        USER_STATES[user_id] = "decrypt_text"
    elif data == "menu_contact":
        await query.edit_message_text("📧 تواصل مع المطور: @AdminUsername", reply_markup=back_to_main_button())
    elif data == "back_main":
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_stats":
        ...
        await query.edit_message_text("أرسل ID المستخدم الذي تريد حظره أو فك حظره:", reply_markup=back_to_main_button())

    elif data == "admin_broadcast":
        USER_STATES[user_id] = "admin_broadcast_msg"
        await query.edit_message_text("أرسل الرسالة التي تريد إرسالها لجميع المستخدمين المفعلين:", reply_markup=back_to_main_button())
...
async def admin_block_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        ...
        new_block_status = 0 if user[1] else 1
        db_execute("UPDATE users SET blocked=? WHERE user_id=?", (new_block_status, target_id))
        status = "حظر" if new_block_status else "فك حظر"
        await update.message.reply_text(f"تم {status} المستخدم {target_id}.")
    USER_STATES.pop(user_id, None)
